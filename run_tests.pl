...
$bin="./draw";
$dataset="/home/ubuntu/datasets/results/SuBSENSE_result/";
...
foreach $key (@files) {
	...
	foreach $key2 (@files2) {
		...
		foreach $key3 (@files3) {
			...
			$filepath = "$dataset/$key/$key2/$key3";
			next if( -d $filepath );
			chomp $filepath;
			...
			print "$filepath\n";
			if( ! -d $dir."/boundingbox") {
				mkdir $dir."/boundingbox";
			}
			$cmd=$bin." ".$filepath." ".$dir."/boundingbox";
			print $cmd."\n";
			system($cmd);
		}
	}
